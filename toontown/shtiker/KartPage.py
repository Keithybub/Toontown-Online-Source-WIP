...
class RacingRecordsUI(DirectFrame):
    ...
    def load(self):
        ...
        for trackId in RaceGlobals.TrackIds:
            ...
            trackNameDisplay = DirectLabel(parent = self, relief = None, text = trackName, text_align = TextNode.ALeft, text_scale = 0.074999999999999997, text_fg = (0.94999999999999996, 0.94999999999999996, 0.0, 1.0), text_shadow = (0, 0, 0, 1), text_pos = (-0.80000000000000004, 0.5 - offset), text_font = ToontownGlobals.getSignFont())
            bestTimeDisplay = DirectLabel(parent = self, relief = None, text = TTLocalizer.KartRace_Unraced, text_scale = 0.059999999999999998, text_fg = (0.0, 0.0, 0.0, 1.0), text_pos = (0.65000000000000002, 0.5 - offset), text_font = ToontownGlobals.getToonFont())
    ...
    def load(self):
        ...
        for j in range(RaceGlobals.NumCups):
            for i in range(RaceGlobals.TrophiesPerCup):
                trophyPanel = DirectLabel(parent = self, relief = None, pos = (xStart + i * xOffset, 0.0, yStart - j * yOffset), state = DGG.NORMAL, image = DGG.getDefaultDialogGeom(), image_scale = (0.75, 1, 1), image_color = (0.80000000000000004, 0.80000000000000004, 0.80000000000000004, 1), text = TTLocalizer.SuitPageMystery[0], text_scale = 0.45000000000000001, text_fg = (0, 0, 0, 1), text_pos = (0, 0, -0.25), text_font = ToontownGlobals.getInterfaceFont(), text_wordwrap = 5.5)
        ...
        self.ticketDisplay = DirectLabel(parent = self, relief = None, image = loader.loadModel('phase_6/models/karting/tickets'), image_pos = (0.20000000000000001, 0, -0.63500000000000001), image_scale = 0.20000000000000001, text = TTLocalizer.KartPageTickets + str(self.avatar.getTickets()), text_scale = 0.070000000000000007, text_fg = (0, 0, 0.94999999999999996, 1.0), text_pos = (0, -0.65000000000000002), text_font = ToontownGlobals.getSignFont())
        self.trophyTextDisplay = DirectLabel(parent = self, relief = None, text = '', text_scale = 0.070000000000000007, text_fg = (1, 0, 0, 1), text_shadow = (0, 0, 0, 0), text_pos = (0.0, -0.17499999999999999), text_font = ToontownGlobals.getInterfaceFont())
        self.updateTrophies()

    
    def grow(self, index, pos):
        self.trophyPanels[index]['image_color'] = Vec4(1.0, 1.0, 0.80000000000000004, 1.0)
...
class ItemSelector(DirectFrame):
    ...
    class ItemViewer(DirectFrame):
        ...
        def load(self, uiRootNode):
            ...
            self.uiImagePlane.component('geom0').setColorScale(1.0, 1.0, 0.80000000000000004, 1.0)
            ...
            self.texCount += 1
            self.uiTextBox = DirectFrame(parent = self, relief = None, scale = 1.0, text = '', text_font = ToontownGlobals.getInterfaceFont(), text_fg = (0.5, 0, 0, 1), text_shadow = (0, 0, 0, 1), text_scale = 0.071499999999999994, text_pos = (0.0, -0.23000000000000001, 0.0))
            self.deleteButton = DirectButton(parent = self, relief = None, geom = (uiRootNode.find('**/uiAccessorydelete_up'), uiRootNode.find('**/uiAccessorydelete_down'), uiRootNode.find('**/uiAccessorydelete_rollover'), uiRootNode.find('**/uiAccessorydelete_rollover')), text = TTLocalizer.KartShtikerDelete, text_font = ToontownGlobals.getSignFont(), text_pos = (0, -0.125, 0), text_scale = TTLocalizer.KPdeleteButton, text_fg = (1, 1, 1, 1), scale = 1.0, pressEffect = False, command = lambda : self._ItemViewer__handleItemDeleteConfirm())
            self.deleteButton.hide()
            self.leftArrowButton = DirectButton(parent = self, relief = None, geom = (uiRootNode.find('**/ArrowLeftButtonUp'), uiRootNode.find('**/ArrowLeftButtonDown'), uiRootNode.find('**/ArrowLeftButtonRollover'), uiRootNode.find('**/ArrowLeftButtonInactive')), scale = 1.0, pressEffect = False, command = lambda : self._ItemViewer__handleItemChange(-1))
            self.rightArrowButton = DirectButton(parent = self, relief = None, geom = (uiRootNode.find('**/ArrowRightButtonUp'), uiRootNode.find('**/ArrowRightButtonDown'), uiRootNode.find('**/ArrowRightButtonRollover'), uiRootNode.find('**/ArrowRightButtonInactive')), scale = 1.0, pressEffect = False, command = lambda : self._ItemViewer__handleItemChange(1))
            ...
            if category == InvalidEntry:
                ...
                if category in colorTypeList:
                    if self.currItem == InvalidEntry or self.currItem not in accessDict.get(KartDNA.bodyColor):
                        ...
                        self._ItemViewer__updateDeleteButton(DGG.NORMAL, TTLocalizer.KartShtikerDelete)
        ...
        def _ItemViewer__updateDeleteButton(self, state, text = TTLocalizer.KartShtikerDelete):
        ...
        def _ItemViewer__handleItemDeleteConfirm(self):
            ...
            if not hasattr(self, 'confirmDlg'):
                uiRootNode = loader.loadModel('phase_6/models/gui/ShtikerBookUI')
                self.confirmDlg = DirectFrame(parent = aspect2d, relief = None, geom = uiRootNode.find('**/uiAccessoryNotice'), geom_scale = 1.0, text = TTLocalizer.KartPageConfirmDelete, text_scale = 0.070000000000000007, text_pos = (0, 0.021999999999999999))
                self.confirmDlg.hide()
                self.confirmDlg.setPos(aspect2d, 0, -0.19500000000000001, -0.19500000000000001)
                ...
                self.confirmButton = DirectButton(parent = self.confirmDlg, relief = None, image = (uiRootNode.find('**/CheckButtonUp'), uiRootNode.find('**/CheckButtonDown'), uiRootNode.find('**/CheckButtonRollover')), geom = uiRootNode.find('**/CheckIcon'), scale = 1.0, pressEffect = False, command = self._ItemViewer__handleItemDelete)
    ...
    def load(self, uiRootNode):
        ...
        self.ebButton = DirectButton(parent = self, relief = None, geom = (uiRootNode.find('**/eBlockButton_up'), uiRootNode.find('**/eBlockButton_rollover'), uiRootNode.find('**/eBlockButton_rollover'), uiRootNode.find('**/eBlockButton_inactive')), scale = 1.0, pressEffect = False, command = lambda : self._ItemSelector__changeItemCategory(KartDNA.ebType))
        self.buttonDict[KartDNA.ebType] = self.ebButton
        self.spButton = DirectButton(parent = self, relief = None, geom = (uiRootNode.find('**/spoilerButton_up'), uiRootNode.find('**/spoilerButton_rollover'), uiRootNode.find('**/spoilerButton_rollover'), uiRootNode.find('**/spoilerButton_inactive')), scale = 1.0, pressEffect = False, command = lambda : self._ItemSelector__changeItemCategory(KartDNA.spType))
        self.buttonDict[KartDNA.spType] = self.spButton
        self.fwwButton = DirectButton(parent = self, relief = None, geom = (uiRootNode.find('**/frontButton_up'), uiRootNode.find('**/frontButton_rollover'), uiRootNode.find('**/frontButton_rollover'), uiRootNode.find('**/frontButton_inactive')), scale = 1.0, pressEffect = False, command = lambda : self._ItemSelector__changeItemCategory(KartDNA.fwwType))
        self.buttonDict[KartDNA.fwwType] = self.fwwButton
        self.bwwButton = DirectButton(parent = self, relief = None, geom = (uiRootNode.find('**/rearButton_up'), uiRootNode.find('**/rearButton_rollover'), uiRootNode.find('**/rearButton_rollover'), uiRootNode.find('**/rearButton_inactive')), scale = 1.0, pressEffect = False, command = lambda : self._ItemSelector__changeItemCategory(KartDNA.bwwType))
        self.buttonDict[KartDNA.bwwType] = self.bwwButton
        self.rimButton = DirectButton(parent = self, relief = None, geom = (uiRootNode.find('**/rimButton_up'), uiRootNode.find('**/rimButton_rollover'), uiRootNode.find('**/rimButton_rollover'), uiRootNode.find('**/rimButton_inactive')), scale = 1.0, pressEffect = False, command = lambda : self._ItemSelector__changeItemCategory(KartDNA.rimsType))
        self.buttonDict[KartDNA.rimsType] = self.rimButton
        self.decalButton = DirectButton(parent = self, relief = None, geom = (uiRootNode.find('**/decalButton_up'), uiRootNode.find('**/decalButton_rollover'), uiRootNode.find('**/decalButton_rollover'), uiRootNode.find('**/decalButton_inactive')), scale = 1.0, pressEffect = False, command = lambda : self._ItemSelector__changeItemCategory(KartDNA.decalType))
        self.buttonDict[KartDNA.decalType] = self.decalButton
        self.paintKartButton = DirectButton(parent = self, relief = None, geom = (uiRootNode.find('**/paintKartButton_up'), uiRootNode.find('**/paintKartButton_rollover'), uiRootNode.find('**/paintKartButton_rollover'), uiRootNode.find('**/paintKartButton_inactive')), scale = 1.0, pressEffect = False, command = lambda : self._ItemSelector__changeItemCategory(KartDNA.bodyColor))
        self.buttonDict[KartDNA.bodyColor] = self.paintKartButton
        self.paintAccessoryButton = DirectButton(parent = self, relief = None, geom = (uiRootNode.find('**/paintAccessoryButton_up'), uiRootNode.find('**/paintAccessoryButton_rollover'), uiRootNode.find('**/paintAccessoryButton_rollover'), uiRootNode.find('**/paintAccessoryButton_inactive')), scale = 1.0, pressEffect = False, command = lambda : self._ItemSelector__changeItemCategory(KartDNA.accColor))
...
class KartViewer(DirectFrame):
    ...
    def load(self, uiRootNode, bgFrame = 'uiKartViewerFrame1', rightArrow = [
        ...
        if leftArrow and len(leftArrow) == 5:
            self.uiRotateLeft = DirectButton(parent = self, relief = None, geom = (uiRootNode.find('**/' + leftArrow[0]), uiRootNode.find('**/' + leftArrow[1]), uiRootNode.find('**/' + leftArrow[2]), uiRootNode.find('**/' + leftArrow[3])), scale = 1.0, text = TTLocalizer.KartView_Left, text_scale = TTLocalizer.KProtateButton, text_pos = (leftArrow[4][0], leftArrow[4][1], 0), text_fg = (1, 1, 1, 1.0), text_shadow = (0, 0, 0, 1), text_font = ToontownGlobals.getSignFont(), pressEffect = False)
            self.uiRotateLeft.bind(DGG.B1PRESS, self._KartViewer__handleKartRotate, [
                -3])
            self.uiRotateLeft.bind(DGG.B1RELEASE, self._KartViewer__endKartRotate)
        
        if rightArrow and len(rightArrow) == 5:
            self.uiRotateRight = DirectButton(parent = self, relief = None, geom = (uiRootNode.find('**/' + rightArrow[0]), uiRootNode.find('**/' + rightArrow[1]), uiRootNode.find('**/' + rightArrow[2]), uiRootNode.find('**/' + rightArrow[3])), scale = 1.0, text = TTLocalizer.KartView_Right, text_scale = TTLocalizer.KProtateButton, text_pos = (rightArrow[4][0], rightArrow[4][1], 0), text_fg = (1, 1, 1, 1.0), text_shadow = (0, 0, 0, 1), text_font = ToontownGlobals.getSignFont(), pressEffect = False)
            self.uiRotateRight.bind(DGG.B1PRESS, self._KartViewer__handleKartRotate, [
                3])
            self.uiRotateRight.bind(DGG.B1RELEASE, self._KartViewer__endKartRotate)
    ...
    def _KartViewer__rotateTask(self, direction):
        if hasattr(self, 'pitch'):
            self.pitch.setH(self.pitch.getH() + 0.40000000000000002 * direction)
            return Task.cont
        else:
            return Task.done
...
class RacingTrophy(DirectFrame):
    ...
    def __init__(self, level, *args, **kwargs):
        ...
        self.nameLabel = DirectLabel(parent = self, relief = None, pos = (0, 0, -0.14999999999999999), text = '', text_scale = 0.125, text_fg = Vec4(0.90000000000000002, 0.90000000000000002, 0.40000000000000002, 1))
    ...
    def setLevel(self, level):
        ...
        if level == -1:
            ...
            if level < 30 and level % 10 == 9:
                ...
                self.greyBowl.hide()
            if level == 30:
                self.goldBowl.setScale(4.4000000000000004, 3.1000000000000001, 3.1000000000000001)
            elif level == 31:
                self.goldBowl.setScale(3.6000000000000001, 3.5, 3.5)
            elif level >= 32:
                self.goldBowl.setScale(5.5999999999999996, 3.8999999999999999, 3.8999999999999999)
    ...
    def _RacingTrophy__bronze(self):
        self.statue.setColorScale(0.90000000000000002, 0.59999999999999998, 0.33000000000000002, 1)

    
    def _RacingTrophy__silver(self):
        self.statue.setColorScale(0.90000000000000002, 0.90000000000000002, 1, 1)

    
    def _RacingTrophy__gold(self):
        self.statue.setColorScale(1, 0.94999999999999996, 0.10000000000000001, 1)
    ...
    def _RacingTrophy__purpleColumn(self):
        self.column.setColorScale(1, 0.69999999999999996, 0.94999999999999996, 1)

    
    def _RacingTrophy__redColumn(self):
        self.column.setColorScale(1.2, 0.59999999999999998, 0.59999999999999998, 1)
