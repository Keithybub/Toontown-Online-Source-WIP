    ...
    def __init__(self, air, playerArray, mpArray, previousMinigameId, trolleyZone, newbieIdList = [], votesArray = None, metagameRound = -1, desiredNextGame = None):
        ...
        for avId in self.getInvolvedPlayerIds():
            if avId > 3 and self.air.doId2do.has_key(avId):
                self.acceptOnce(self.air.getAvatarExitEvent(avId), self._PurchaseManagerAI__handleUnexpectedExit, extraArgs = [
                    avId])
                ...
                self.air.writeServerEvent('minigame', avId, '%s|%s|%s|%s' % (self.previousMinigameId, self.trolleyZone, self.playerIds, self.minigamePoints[avIndex]))
                if self.metagameRound == TravelGameGlobals.FinalMetagameRoundIndex:
                    ...
                    self.air.writeServerEvent('minigame_extraBeans', avId, '%s|%s|%s|%s' % (self.previousMinigameId, self.trolleyZone, self.playerIds, extraBeans))
