...
class DisguisePage(ShtikerPage.ShtikerPage):
    ...
    def load(self):
        ...
        for dept in SuitDNA.suitDepts:
            ...
            tab = DirectButton(parent = self.pageFrame, relief = None, geom = tabGeom, geom_color = DeptColors[tabIndex - 1], text = SuitDNA.suitDeptFullnames[dept], text_font = ToontownGlobals.getSuitFont(), text_pos = textPos, text_roll = -90, text_scale = TTLocalizer.DPtab, text_align = TextNode.ACenter, text1_fg = Vec4(1, 0, 0, 1), text2_fg = Vec4(0.5, 0.40000000000000002, 0.40000000000000002, 1), text3_fg = Vec4(0.40000000000000002, 0.40000000000000002, 0.40000000000000002, 1), command = self.doTab, extraArgs = [
                len(self.tabs)], pressEffect = 0)
            self.tabs.append(tab)
            page = DirectFrame(parent = tab, relief = None, geom = pageGeom)
        
        self.deptLabel = DirectLabel(parent = self.frame, text = '', text_font = ToontownGlobals.getSuitFont(), text_scale = TTLocalizer.DPdeptLabel, text_pos = (-0.10000000000000001, 0.80000000000000004))
        ...
        self.juryNoticeTitle = DirectLabel(parent = self.frame, relief = None, geom = gui.find('**/text_jury_notice_progress'), geom_pos = (0, 0.10000000000000001, 0))
        self.juryNoticeTitle.hide()
        self.stockOptionTitle = DirectLabel(parent = self.frame, relief = None, geom = gui.find('**/text_stock_option_progress'), geom_pos = (0, 0.10000000000000001, 0))
        ...
        self.cogName = DirectLabel(parent = self.frame, relief = None, text = '', text_font = ToontownGlobals.getSuitFont(), text_scale = TTLocalizer.DPcogName, text_align = TextNode.ACenter, pos = (-0.94799999999999995, 0, -1.1499999999999999))
