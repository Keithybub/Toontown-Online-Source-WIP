...
class IssueFrame(DirectFrame):
    ...
    def createPage(self, section, subsection, fullFilename):
        ...
        pageFrame = DirectFrame(frameColor = (1, 1, 1, 0), frameSize = self.FrameDimensions, image = upsellBackground, image_scale = (imageScaleX, 1, imageScaleY), relief = DGG.FLAT, parent = self, text = '', text_scale = 0.059999999999999998, text_pos = (0, -0.40000000000000002))
    ...
    def loadRightArrow(self, section, subsection, pageFrame):
        ...
        if nextTarget:
            ...
            rightArrow = DirectButton(relief = None, parent = pageFrame, command = self.gotoPage, extraArgs = (nextTarget[0], nextTarget[1]), image = (image, image, rollover, image), pos = position, image_scale = imageScale)
    ...
    def loadLeftArrow(self, section, subsection, pageFrame):
        ...
        if prevTarget:
            ...
            rightArrow = DirectButton(relief = None, parent = pageFrame, command = self.gotoPage, extraArgs = (prevTarget[0], prevTarget[1]), image = (image, image, rollover, image), pos = position, image_scale = imageScale)
    ...
    def loadHomePageButtons(self, section, subsection, pageFrame):
        ...
        for section in xrange(1, len(self.SectionIdents)):
            ...
            sectionBtn = DirectButton(relief = None, parent = pageFrame, image = (image, image, rolloverImage, image), image_scale = image_scale, command = self.gotoPage, extraArgs = (section, 0), enableEdit = 1, pos = positions[section])
        ...
        readMoreBtn = DirectButton(relief = None, parent = pageFrame, image = (readImage, readImage, readRollover, readImage), image_scale = imageScale, command = self.gotoPage, extraArgs = (1, 0), enableEdit = 1, pos = readMorePos)
    ...
    def loadWeekNavButtons(self, pageFrame):
        ...
        prevWeekBtn = DirectButton(relief = None, parent = pageFrame, image = [
            prevImage,
            prevImage,
            prevImageRo,
            prevImageDisabled], image_scale = y1Scale, command = self.changeWeek, extraArgs = (self.myIssueIndex - 1,), pos = (0.80666599999999999, 0, 0.62))
        ...
        nextWeekBtn = DirectButton(relief = None, parent = pageFrame, image = [
            nextImage,
            nextImage,
            nextImageRo,
            nextImageDisabled], image_scale = y2Scale, command = self.changeWeek, extraArgs = (self.myIssueIndex + 1,), pos = (1.1599999999999999, 0, 0.62333300000000003))
        if self.myIssueIndex == self.numIssues - 1:
        ...
        weekLabel = DirectLabel(relief = None, image = midImage, image_scale = imageScale, parent = pageFrame, text = weekStr, text_font = ToontownGlobals.InterfaceFont, text_fg = weekColor, text_scale = 0.042999999999999997, text_pos = (0, -0.01, 0), pos = (0.98333300000000001, 0, 0.62))
    ...
    def loadNavButtons(self, pageFrame):
        ...
        frameSize1 = (-0.040000000000000001 + frameSizeAdj1, 0.040000000000000001 + frameSizeAdj1, -0.040000000000000001, 0.040000000000000001)
        frameSizeAdj2 = 0.20999999999999999
        frameSize2 = (-0.040000000000000001 + frameSizeAdj2, 0.040000000000000001 + frameSizeAdj2, -0.040000000000000001, 0.040000000000000001)
        frameSizes = (frameSize1, frameSize1, frameSize1, frameSize2, frameSize2, frameSize2)
        self.sectionBtns = []
        for section in xrange(0, len(self.SectionIdents)):
            ...
            sectionBtn = DirectButton(relief = None, parent = pageFrame, frameSize = frameSizes[section], image = (image, rolloverImage, rolloverImage, image), image_scale = imageScales[section], command = self.gotoPage, extraArgs = (section, 0), enableEdit = 1, pos = rolloverPositions[section])
