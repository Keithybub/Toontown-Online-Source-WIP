...
class HalloweenHolidayDecorator(HolidayDecorator.HolidayDecorator):
    ...
    def _HalloweenHolidayDecorator__checkHoodValidity(self):
        if hasattr(base.cr.playGame, 'getPlace') and base.cr.playGame.getPlace():
            if (isinstance(base.cr.playGame.getPlace(), Playground.Playground) or isinstance(base.cr.playGame.getPlace(), Estate.Estate)) and hasattr(base.cr.playGame.getPlace(), 'loader') and base.cr.playGame.getPlace().loader and hasattr(base.cr.playGame.getPlace().loader, 'hood') and base.cr.playGame.getPlace().loader.hood and hasattr(base.cr.playGame.getPlace().loader.hood, 'loader') and base.cr.playGame.getPlace().loader.hood.loader and hasattr(base.cr.playGame.getPlace().loader.hood.loader, 'geom') and base.cr.playGame.getPlace().loader.hood.loader.geom:
                return True
            elif hasattr(base.cr.playGame, 'getPlace') and base.cr.playGame.getPlace():
    ...
    def _HalloweenHolidayDecorator__startSpookySky(self):
        if (self._HalloweenHolidayDecorator__checkHoodValidity() or self._HalloweenHolidayDecorator__checkStreetValidity()) and hasattr(base.cr.playGame.hood, 'sky') and base.cr.playGame.hood.sky:
    ...
    def decorate(self):
        ...
        if (self._HalloweenHolidayDecorator__checkHoodValidity() or self._HalloweenHolidayDecorator__checkStreetValidity()) and hasattr(base.cr.playGame, 'hood') and base.cr.playGame.hood and hasattr(base.cr.playGame.hood, 'sky') and base.cr.playGame.hood.sky:
            preShow = Sequence(Parallel(LerpColorScaleInterval(base.cr.playGame.hood.sky, 1.5, Vec4(1, 1, 1, 0.25)), LerpColorScaleInterval(base.cr.playGame.hood.loader.geom, 2.5, Vec4(0.55000000000000004, 0.55000000000000004, 0.65000000000000002, 1)), Func(__lightDecorationOn__)), Func(self._HalloweenHolidayDecorator__startSpookySky))
            preShow.start()
    ...
    def undecorate(self):
        if (self._HalloweenHolidayDecorator__checkHoodValidity() or self._HalloweenHolidayDecorator__checkStreetValidity()) and hasattr(base.cr.playGame.hood, 'sky') and base.cr.playGame.hood.sky:
            postShow = Sequence(Parallel(LerpColorScaleInterval(base.cr.playGame.hood.sky, 1.5, Vec4(1, 1, 1, 1)), LerpColorScaleInterval(base.cr.playGame.hood.loader.geom, 2.5, Vec4(1, 1, 1, 1))), Func(self._HalloweenHolidayDecorator__stopSpookySky))
            postShow.start()
