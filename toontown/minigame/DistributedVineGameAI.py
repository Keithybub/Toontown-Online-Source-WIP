...
class DistributedVineGameAI(DistributedMinigameAI):
    ...
    def gameOver(self):
        ...
        self.air.writeServerEvent('minigame_vine', self.doId, '%s|%s|%s|%s|%s|%s|%s|%s|%s|%s' % (ToontownGlobals.VineGameId, self.getSafezoneId(), self.avIdList, scoreList, self.vineSections, finishedList, timeLeftList, vineReached, self.totalSpiders, totalBats))
        self.gameFSM.request('cleanup')
    ...
    def enterWaitShowScores(self):
        ...
        taskMgr.doMethodLater(VineGameGlobals.ShowScoresDuration, self._DistributedVineGameAI__doneShowingScores, self.taskName('waitShowScores'))
