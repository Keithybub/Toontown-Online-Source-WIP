...
class TwoDEnemy(DirectObject):
    ...
    def exitPause(self):
        if hasattr(self, 'moveIval') and self.moveIval:
            ...
            if self.isMovingLeftRight:
                ...
                self.propTrack.loop()
                self.animTrack.loop()
                base.playSfx(self.suitSound, node = self.suit, looping = 1, volume = 0.10000000000000001)
    ...
    def doDeathTrack(self):
        ...
        if self.isMovingLeftRight:
            ...
            soundTrack = Sequence(SoundInterval(spinningSound, duration = 1.6000000000000001, startTime = 0.59999999999999998, volume = 0.80000000000000004, node = self.deathSuit), SoundInterval(deathSound, volume = 0.32000000000000001, node = self.deathSuit))
            gears1Track = Sequence(ParticleInterval(smallGears, self.deathSuit, worldRelative = 0, duration = 4.2999999999999998, cleanup = True), name = 'gears1Track')
            gears2MTrack = Track((0.0, explosionTrack), (0.69999999999999996, ParticleInterval(singleGear, self.deathSuit, worldRelative = 0, duration = 5.7000000000000002, cleanup = True)), (5.2000000000000002, ParticleInterval(smallGearExplosion, self.deathSuit, worldRelative = 0, duration = 1.2, cleanup = True)), (5.4000000000000004, ParticleInterval(bigGearExplosion, self.deathSuit, worldRelative = 0, duration = 1.0, cleanup = True)), name = 'gears2MTrack')
        elif self.isMovingUpDown:
            ...
            soundTrack = Sequence(SoundInterval(spinningSound, duration = 1.6000000000000001, startTime = 0.59999999999999998, volume = 0.80000000000000004, node = self.deathSuit), SoundInterval(deathSound, volume = 0.32000000000000001, node = self.deathSuit))
            gears1Track = Sequence(ParticleInterval(smallGears, self.deathSuit, worldRelative = 0, duration = 4.2999999999999998, cleanup = True), name = 'gears1Track')
            gears2MTrack = Track((0.0, explosionTrack), (0.0, ParticleInterval(singleGear, self.deathSuit, worldRelative = 0, duration = 5.7000000000000002, cleanup = True)), (2.7000000000000002, ParticleInterval(smallGearExplosion, self.deathSuit, worldRelative = 0, duration = 1.2, cleanup = True)), (2.8999999999999999, ParticleInterval(bigGearExplosion, self.deathSuit, worldRelative = 0, duration = 1.0, cleanup = True)), name = 'gears2MTrack')
