...
class DistributedPatternGame(DistributedMinigame):
    ...
    ButtonSoundNames = (phase4snd + 'm_match_trumpet.mp3', phase4snd + 'm_match_guitar.mp3', phase4snd + 'm_match_drums.mp3', phase4snd + 'm_match_piano.mp3')
    ...
    def __init__(self, cr):
        ...
        self.normalTextColor = VBase4(0.53700000000000003, 0.83999999999999997, 0.33000000000000002, 1.0)
    ...
    def load(self):
        ...
        self.roundText = DirectLabel(text = self.strRound % 1, text_fg = self.normalTextColor, frameColor = (1, 1, 1, 0), text_font = ToontownGlobals.getSignFont(), pos = (0.014, 0, -0.83999999999999997), scale = 0.12)
        self.roundText.hide()
    ...
    def onstage(self):
        ...
        for anim in self.minnieAnimNames:
            ...
            self.minnieAnimSpeedMult[anim] = float(self._DistributedPatternGame__numPingPongFrames(numFrames)) / float(self.stdNumDanceStepPingPongFrames)
        
        for anim in self.toonAnimNames:
            numFrames = self.lt.getNumFrames(anim)
            self.toonAnimSpeedMult[anim] = float(self._DistributedPatternGame__numPingPongFrames(numFrames)) / float(self.stdNumDanceStepPingPongFrames)
    ...
    def _DistributedPatternGame__getDanceStepAnimTrack(self, toon, anim, speedScale):
    ...
    def _DistributedPatternGame__getMinnieDanceStepAnimTrack(self, minnie, direction):
        animName = self.minnieAnimNames[direction]
        return self._DistributedPatternGame__getDanceStepAnimTrack(minnie, animName, self.minnieAnimSpeedMult[animName])

    
    def _DistributedPatternGame__getToonDanceStepAnimTrack(self, toon, direction):
        animName = self.toonAnimNames[direction]
        return self._DistributedPatternGame__getDanceStepAnimTrack(toon, animName, self.toonAnimSpeedMult[animName])

    
    def getDanceStepButtonSoundTrack(self, index):
        duration = self.getDanceStepDuration()
        wait = duration * self.buttonPressDelayPercent
        return Sequence(Wait(wait), Func(base.playSfx, self._DistributedPatternGame__getButtonSound(index)), Wait(duration - wait))
    ...
    def _DistributedPatternGame__getRowPos(self, rowHome, xSpacing, index, numSpots):
    ...
    def getBackRowPos(self, avId):
        ...
        return self._DistributedPatternGame__getRowPos(self.backRowHome, self.backRowXSpacing, index, len(self.remoteAvIdList))

    
    def getFrontRowPos(self, avId):
        index = self.avIdList.index(avId)
        return self._DistributedPatternGame__getRowPos(self.frontRowHome, self.frontRowXSpacing, index, len(self.avIdList))
    ...
    def enterShowServerPattern(self):
        ...
        text = self.strWatch
        danceTrack = self.getDanceSequenceAnimTrack(self.minnie, self._DistributedPatternGame__serverPattern)
        arrowTrack = self.getDanceArrowAnimTrack('m', self._DistributedPatternGame__serverPattern, 0)
        soundTrack = self.getDanceSequenceButtonSoundTrack(self._DistributedPatternGame__serverPattern)
        self.showTrack = Sequence(Func(self._DistributedPatternGame__setMinnieChat, text, 1), Wait(0.5), Parallel(danceTrack, soundTrack, arrowTrack), Wait(0.20000000000000001), Func(self._DistributedPatternGame__clearMinnieChat), Func(self.gameFSM.request, 'getUserInput'))
    ...
    def enterGetUserInput(self):
        ...
        def startTimer(self = self):
            ...
            self.timer.show()
            self.timer.countdown(PatternGameGlobals.InputTime, self._DistributedPatternGame__handleInputTimeout)
        ...
        self.setupTrack = Sequence(Func(self._DistributedPatternGame__setMinnieChat, self.strGo, 0), Func(self.setText, self.roundText, TTLocalizer.PatternGameGo), Func(self.roundText.setScale, 0.29999999999999999), Func(enableKeys), Func(startTimer), Wait(0.80000000000000004), Func(self._DistributedPatternGame__clearMinnieChat), Func(self.setText, self.roundText, ' '), Func(self.roundText.setScale, 0.12), Func(self.setTextFG, self.roundText, self.normalTextColor))
    ...
    def _DistributedPatternGame__doneGettingInput(self, pattern):
        ...
        self.proceedTrack = Sequence(Wait(self.getDanceStepDuration()), Func(self.sendUpdate, 'reportPlayerPattern', [
            pattern,
            self.currentTotalTime]), Func(self.gameFSM.request, 'waitForPlayerPatterns'))
    ...
    def enterPlayBackPatterns(self):
        ...
        if success:
            ...
            text = self.strWrong
        soundTrack = Sequence(Func(base.playSfx, sound), Wait(1.6000000000000001))
        textTrack = Sequence(Wait(0.20000000000000001), Func(self._DistributedPatternGame__setMinnieChat, text, 0), Wait(1.3), Func(self._DistributedPatternGame__clearMinnieChat))
    ...
    def enterCheckGameOver(self):
        ...
        if self.round < PatternGameGlobals.NUM_ROUNDS:
            ...
            if self.celebrate:
                ...
                self._DistributedPatternGame__winTrack = Sequence(Func(self._DistributedPatternGame__setMinnieChat, text, 1), Func(base.playSfx, self.perfectSound), Sequence(self.returnCelebrationIntervals(1)), Sequence(self.returnCelebrationIntervals(0)), Func(self._DistributedPatternGame__clearMinnieChat), Func(self.gameOver))
            else:
                self._DistributedPatternGame__winTrack = Sequence(Func(self._DistributedPatternGame__setMinnieChat, text, 1), Func(base.playSfx, sound), Wait(delay), Func(self._DistributedPatternGame__clearMinnieChat), Func(self.gameOver))
