...
class DistributedPhotoGame(DistributedMinigame, PhotoGameBase.PhotoGameBase):
    ...
    def load(self):
        ...
        self.filmPanel = DirectLabel(parent = hidden, relief = None, pos = (1.1599999999999999, 0.0, 0.45000000000000001), scale = 0.65000000000000002, text = str(self.filmCount), text_scale = 0.20000000000000001, text_fg = (0.94999999999999996, 0.94999999999999996, 0, 1), text_pos = (0.080000000000000002, -0.14999999999999999), text_font = ToontownGlobals.getSignFont(), image = self.filmImage, image_scale = Point3(1.0, 0.0, 0.84999999999999998))
        self.filmPanelTitle = DirectLabel(parent = self.filmPanel, relief = None, pos = (0.080000000000000002, 0, 0.040000000000000001), scale = 0.080000000000000002, text = TTLocalizer.PhotoGameFilm, text_fg = (0.94999999999999996, 0.94999999999999996, 0, 1), text_shadow = (0, 0, 0, 1))
        ...
        BuildGeometry.addSquareGeom(self.blackoutNode, self.screenSizeX * 2.2000000000000002, self.screenSizeZ * 2.2000000000000002, Vec4(1.0, 1.0, 1.0, 1.0))
            ...
            if content:
                ...
                if score and photoAnalysisZero in self.assignments:
                    ...
                    if score >= self.assignmentDataDict[assignment][0]:
                        ...
                        taskMgr.doMethodLater(0.20000000000000001, buffer.setActive, 'capture Image', [
                            0])
                        if score > self.assignmentDataDict[assignment][0]:
    ...
    def setGameStart(self, timestamp):
        ...
        if not base.config.GetBool('endless-cannon-game', 0):
            self.timer.show()
            self.timer.countdown(self.data['TIME'], self._DistributedPhotoGame__gameTimerExpired)
    ...
    def generateAssignmentPanels(self):
        ...
        spacing = (self.screenSizeX / PhotoGameGlobals.ONSCREENASSIGNMENTS) * 1.6100000000000001
        index = 0
    ...
    def makeAssignmentPanel(self, assignment):
        ...
        BuildGeometry.addSquareGeom(screen, 0.35999999999999999, 0.27000000000000002, Vec4(1.0, 1.0, 1.0, 1.0))
    ...
    def makeFrameModel(self, model):
        ...
        if model:
            ...
            scale.setScale(0.40000000000000002 * scaleFactor / max(corner[0], corner[1], corner[2]))
    ...
    def makeAssignmentFrame(self):
        ...
        frame = DirectFrame(parent = hidden, image = photoImage, image_color = (1, 1, 1, 1), image_scale = Point3(1.6000000000000001 * assignmentScale, 0.0, 1.75 * assignmentScale), frameSize = (-size, size, -size, size), text = 'HC Score', textMayChange = 1, text_wordwrap = 9, text_pos = Point3(0.0, -0.13500000000000001, 0.0), text_scale = 0.044999999999999998, relief = None)
        return frame
    ...
    def makeScoreFrame(self):
        ...
        frame = DirectFrame(parent = hidden, image = scoreImage, image_color = (1, 1, 1, 1), image_scale = Point3(0.64000000000000001, 0.0, 0.64000000000000001), frameSize = (-size, size, -size, size), text = 'Score Frame', textMayChange = 1, text_wordwrap = 9, text_pos = Point3(0.0, 0.0, 0.0), text_scale = 0.050000000000000003, relief = None)
        return frame
    ...
    def enterAim(self):
        ...
        taskMgr.add(self._DistributedPhotoGame__moveViewfinder, 'photo game viewfinder Task')
    ...
    def _DistributedPhotoGame__enableAimInterface(self):
        ...
        self.accept(self.RIGHT_KEY, self._DistributedPhotoGame__rightKeyPressed)
    ...
    def _DistributedPhotoGame__fireKeyPressed(self):
        self.ignore(self.FIRE_KEY)
        self.accept(self.FIRE_KEY + '-up', self._DistributedPhotoGame__fireKeyReleased)
        self._DistributedPhotoGame__firePressed()

    
    def _DistributedPhotoGame__upKeyPressed(self):
        self.ignore(self.UP_KEY)
        self.accept(self.UP_KEY + '-up', self._DistributedPhotoGame__upKeyReleased)
        self._DistributedPhotoGame__upPressed()

    
    def _DistributedPhotoGame__downKeyPressed(self):
        self.ignore(self.DOWN_KEY)
        self.accept(self.DOWN_KEY + '-up', self._DistributedPhotoGame__downKeyReleased)
        self._DistributedPhotoGame__downPressed()

    
    def _DistributedPhotoGame__leftKeyPressed(self):
        self.ignore(self.LEFT_KEY)
        self.accept(self.LEFT_KEY + '-up', self._DistributedPhotoGame__leftKeyReleased)
        self._DistributedPhotoGame__leftPressed()

    
    def _DistributedPhotoGame__rightKeyPressed(self):
        self.ignore(self.RIGHT_KEY)
        self.accept(self.RIGHT_KEY + '-up', self._DistributedPhotoGame__rightKeyReleased)
        self._DistributedPhotoGame__rightPressed()
    ...
    def _DistributedPhotoGame__upPressed(self):
        ...
        self.upPressed = self._DistributedPhotoGame__enterControlActive(self.upPressed)

    
    def _DistributedPhotoGame__downPressed(self):
        self.notify.debug('down pressed')
        self.downPressed = self._DistributedPhotoGame__enterControlActive(self.downPressed)

    
    def _DistributedPhotoGame__leftPressed(self):
        self.notify.debug('left pressed')
        self.leftPressed = self._DistributedPhotoGame__enterControlActive(self.leftPressed)

    
    def _DistributedPhotoGame__rightPressed(self):
        self.notify.debug('right pressed')
        self.rightPressed = self._DistributedPhotoGame__enterControlActive(self.rightPressed)

    
    def _DistributedPhotoGame__upReleased(self):
        self.notify.debug('up released')
        self.upPressed = self._DistributedPhotoGame__exitControlActive(self.upPressed)

    
    def _DistributedPhotoGame__downReleased(self):
        self.notify.debug('down released')
        self.downPressed = self._DistributedPhotoGame__exitControlActive(self.downPressed)

    
    def _DistributedPhotoGame__leftReleased(self):
        self.notify.debug('left released')
        self.leftPressed = self._DistributedPhotoGame__exitControlActive(self.leftPressed)

    
    def _DistributedPhotoGame__rightReleased(self):
        self.notify.debug('right released')
        self.rightPressed = self._DistributedPhotoGame__exitControlActive(self.rightPressed)
    ...
    def _DistributedPhotoGame__decreaseFilmCount(self):
        ...
        if self.curScore != score:
            ...
            self.jarIval = Parallel(Sequence(self.filmPanel.scaleInterval(0.14999999999999999, s * 3.0 / 4.0, blendType = 'easeOut'), self.filmPanel.scaleInterval(0.14999999999999999, s, blendType = 'easeIn')), Sequence(Wait(0.25), SoundInterval(self.sndFilmTick)), name = 'photoGameFilmJarThrob')
            self.jarIval.start()
    ...
    def constructDD(self):
        ...
        ddFog.setColor(Vec4(0.80000000000000004, 0.80000000000000004, 0.80000000000000004, 1.0))
    ...
    def constructDL(self):
        ...
        self.photoRoot.setPos(-70.227999999999994, 87.587999999999994, 4.3970000000000002)
