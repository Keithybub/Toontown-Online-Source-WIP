...
global infinityTOffsets, trackListGenFuncs
import math
...
def getRandomRingTrackGroup(type, numRings, rng):
    ...
    trackGroup = RingTrackGroup.RingTrackGroup(tracks, period, trackTOffsets = tOffsets, reverseFlag = rng.choice([
        0,
        1]), tOffset = rng.random())
    return trackGroup
...
def get_keypad(numRings, rng):
    positions = (RingTracks.center, RingTracks.up, RingTracks.down, RingTracks.left, RingTracks.right, RingTracks.ul, RingTracks.ur, RingTracks.lr, RingTracks.ll)
    tracks = []
    usedPositions = [
        None]
