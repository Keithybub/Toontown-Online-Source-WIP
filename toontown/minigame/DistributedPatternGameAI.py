...
class DistributedPatternGameAI(DistributedMinigameAI):
    ...
    def enterWaitForResults(self):
        ...
        self.fastestAvId = 0
        self.resultsBarrier = ToonBarrier('results', self.uniqueName('results'), self.avIdList, PatternGameGlobals.InputTimeout + 1.0 * self.round, self._DistributedPatternGameAI__gotAllPatterns, self._DistributedPatternGameAI__resultsTimeout)
    ...
    def _DistributedPatternGameAI__resultsTimeout(self, avIds):
        ...
        for avId in avIds:
            ...
            self._DistributedPatternGameAI__acceptPlayerPattern(avId, [], PatternGameGlobals.InputTime * 2)
    ...
    def reportPlayerPattern(self, pattern, totalTime):
        ...
        avId = self.air.getAvatarIdFromSender()
        self._DistributedPatternGameAI__acceptPlayerPattern(avId, pattern, totalTime)
        self.resultsBarrier.clear(avId)

    
    def _DistributedPatternGameAI__acceptPlayerPattern(self, avId, pattern, totalTime):
        index = self.avIdList.index(avId)
