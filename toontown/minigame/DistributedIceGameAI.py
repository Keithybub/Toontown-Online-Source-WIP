...
class DistributedIceGameAI(DistributedMinigameAI.DistributedMinigameAI):
    ...
    def enterFinalResults(self):
        ...
        self.sendUpdate('setNewState', [
            'finalResults'])
        taskMgr.doMethodLater(IceGameGlobals.ShowScoresDuration, self._DistributedIceGameAI__doneShowingScores, self.taskName('waitShowScores'))
    ...
    def endingPositions(self, positions):
        if not (self.gameFSM) and not self.gameFSM.getCurrentState() or self.gameFSM.getCurrentState().getName() != 'waitEndingPositions':
            return None
    ...
    def reportScoringMovieDone(self):
        if not (self.gameFSM) and not self.gameFSM.getCurrentState() or self.gameFSM.getCurrentState().getName() != 'scoreMatch':
            return None
    ...
    def claimTreasure(self, treasureNum):
        ...
        if not self.scoreDict.has_key(avId):
            self.notify.warning('PROBLEM: avatar %s called claimTreasure(%s) but he is not in the scoreDict: %s. avIdList is: %s' % (avId, treasureNum, self.scoreDict, self.avIdList))
            return None
    ...
    def claimPenalty(self, penaltyNum):
        ...
        avId = self.air.getAvatarIdFromSender()
        if not self.scoreDict.has_key(avId):
            self.notify.warning('PROBLEM: avatar %s called claimPenalty(%s) but he is not in the scoreDict: %s. avIdList is: %s' % (avId, penaltyNum, self.scoreDict, self.avIdList))
            return None
