...
class TwoDStomper(DirectObject):
    ...
    def getSmokeTrack(self):
        smokeTrack = Sequence(Parallel(LerpScaleInterval(self.smoke, 0.20000000000000001, Point3(1, 1, 1.5)), LerpColorScaleInterval(self.smoke, 0.40000000000000002, VBase4(1, 1, 1, 0), VBase4(1, 1, 1, 0.5))), Func(self.smoke.reparentTo, hidden), Func(self.smoke.clearColorScale))
        return smokeTrack
    ...
    def start(self, elapsedTime):
        ...
        self.ival = Parallel()
        self.ival.append(Sequence(self.getMotionIval(), Func(base.playSfx, self.stompSound, node = self.model, volume = 0.29999999999999999), Func(self.smoke.reparentTo, self.model), self.getSmokeTrack()))
