...
class DistributedDivingGame(DistributedMinigame):
    ...
    def fishSpawn(self, timestamp, fishcode, spawnerId, offset):
        ...
        if fish.name == 'clown':
            ...
            fish.specialLerp = LerpFunc(self.pbjMove, duration = 5 * self.SPEEDMULT * self.LAG_COMP, fromData = 0, toData = 2.0 * 3.1415899999999999, extraArgs = [
                fish,
                self.spawners[spawnerId].position.getZ()], blendType = 'easeInOut')
            ...
            fish.specialLerp = Sequence(Wait((offset / 10.0) * 2 + 1.5), Parallel(LerpScaleInterval(fish, duration = 0.29999999999999999, startScale = Vec3(2, 2, 2), scale = Vec3(5, 3, 5), blendType = 'easeInOut')), Wait(1.0), Parallel(LerpScaleInterval(fish, duration = 0.40000000000000002, startScale = Vec3(5, 3, 5), scale = Vec3(2, 2, 2), blendType = 'easeInOut')))
    ...
    def onstage(self):
        ...
        self.environModel.setScale(2.7999999999999998, 2.7999999999999998, 2.73)
    ...
    def setGameReady(self):
        ...
        for i in range(self.NUMTREASURES):
            ...
            self.accept('grab-' + str(i), self._DistributedDivingGame__treasureGrabbed)
    ...
    def _DistributedDivingGame__initPosBroadcast(self):
        ...
        self._DistributedDivingGame__lastPosBroadcast = self.getAvatar(self.localAvId).getPos()
    ...
    def _DistributedDivingGame__spawnTreasureBoundsTask(self):
        ...
        taskMgr.add(self._DistributedDivingGame__treasureBoundsTask, self.TREASURE_BOUNDS_TASK)
        ...
        if toonSD.status == 'treasure' and self.treasures and self.chestIcons:
            for i in range(self.NUMTREASURES):
                if self.treasures[i].grabbedId == avId:
                    ...
                    jump = ProjectileInterval(self.treasures[i].treasureNode, duration = 1.0, startPos = self.treasures[i].treasureNode.getPos(), endPos = Point3(0, 0, 40), gravityMult = 0.69999999999999996)
        ...
        if self.spawners[spawnerId].fishArray.has_key(spawnId):
            ...
            if fish.name == 'clown' or fish.name == 'piano':
                if fish.name != 'piano':
                    ...
                    endHpr = Vec3(0, 0, 0)
                fish.moveLerp = Sequence(LerpHprInterval(fish, duration = 0.40000000000000002, startHpr = fish.getHpr(), hpr = endHpr), LerpFunc(fish.setX, duration = 1.5, fromData = fish.getX(), toData = endX), Func(self.fishRemove, str(spawnerId) + str(spawnId)))
            elif fish.name == 'pbj':
                ...
                fish.moveLerp = Sequence(Func(fish.stop, 'anim'), speed, anim, Wait(1.0), LerpScaleInterval(fish, duration = 0.80000000000000004, startScale = fish.getScale, scale = 0.001, blendType = 'easeIn'), Func(self.fishRemove, str(spawnerId) + str(spawnId)))
    ...
    def setTreasureGrabbed(self, avId, chestId):
        ...
        if toonSD and toonSD.status == 'normal':
            ...
            pos = headparts[2].getPos()
            self.treasures[chestId].treasureNode.setPos(pos + Point3(0, 0.20000000000000001, 3))
    ...
    def setCrabMoving(self, crabId, timestamp, rand1, rand2, crabX, dir):
        ...
        crabSoundInterval = SoundInterval(self.crabSound, loop = 0, duration = 1.6000000000000001, startTime = 0.0)
    ...
    def grabCrapVolume(self, crab):
        if crab:
            ...
            if distance < soundRange:
                crabVolume = (soundRange - distance) / soundRange
                crabSoundInterval = SoundInterval(self.crabSound, loop = 0, duration = 1.6000000000000001, startTime = 0.0, volume = crabVolume)
    ...
    def _DistributedDivingGame__spawnUpdateLocalToonTask(self):
        ...
        taskMgr.add(self._DistributedDivingGame__updateLocalToonTask, self.UPDATE_LOCALTOON_TASK)
