...
class DistributedCogThiefGameAI(DistributedMinigameAI.DistributedMinigameAI):
    ...
    def b_makeCogCarryBarrel(self, clientStamp, cogIndex, barrelIndex):
        ...
        self.notify.debug('b_makeCogCarryBarrel timeStamp=%s clientStamp=%s cog=%s barrel=%s' % (timestamp, clientStamp, cogIndex, barrelIndex))
    ...
    def startSuitGoals(self):
        ...
        for cogIndex in xrange(self.getNumCogs()):
            self.doMethodLater(delayTimes[cogIndex], self.chooseSuitGoal, self.uniqueName('choseSuitGoal-%d-' % cogIndex), extraArgs = [
                cogIndex])
    ...
    def hitBySuit(self, avId, timestamp, suitNum, x, y, z):
        ...
        self.doMethodLater(self.ExplodeWaitTime, self.chooseSuitGoal, self.uniqueName('choseSuitGoal-%d-' % suitNum), extraArgs = [
            suitNum])
    ...
    def cogAtReturnPos(self, clientstamp, cogIndex, barrelIndex):
        ...
        if self.cogInfo[cogIndex]['goal'] == CTGG.RunAwayGoal:
            if self.isCogCarryingThisBarrel(cogIndex, barrelIndex):
                ...
                self.doMethodLater(0.5, self.chooseSuitGoal, self.uniqueName('choseSuitGoal-%d-' % cogIndex), extraArgs = [
                    cogIndex])
