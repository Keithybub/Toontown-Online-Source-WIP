...
class DistributedTugOfWarGame(DistributedMinigame):
    ...
    def onstage(self):
        ...
        camera.setPosHpr(-11.4427, 9.0355899999999991, 2.8009400000000002, -49.103999999999999, -0.89000000000000001, 0)
    ...
    def arrangeByHeight(self, avIdList, order, iStart, iFin):
        for i in range(iStart, iFin + 1):
            for j in range(i + 1, iFin + 1):
                if (order == self.H_TO_L or self.rightHandDict[avIdList[i]].getZ() < self.rightHandDict[avIdList[j]].getZ() or order == self.L_TO_H) and self.rightHandDict[avIdList[i]].getZ() > self.rightHandDict[avIdList[j]].getZ():
                    temp = avIdList[i]
    ...
    def sendGoSignal(self, index):
        ...
        self.setupTrack = Sequence(Func(self.setText, self.roundText, TTLocalizer.TugOfWarGameReady), Wait(1.5), Func(base.playSfx, self.whistleSound), Func(self.setText, self.roundText, TTLocalizer.TugOfWarGameGo), Func(self.roundText.setScale, 0.29999999999999999), Wait(1.5), Func(startTimer), Func(enableKeys), Func(self.gameFSM.request, 'tug'), Func(self.setText, self.roundText, ' '), Func(self.roundText.setScale, 0.20000000000000001))
    ...
    def checkIfFallen(self, avId = None):
        if avId == None:
            if self.suitId not in self.fallenList:
                curPos = self.suit.getPos()
                if (curPos[0] < 0 or curPos[0] > -2 or curPos[0] > 0) and curPos[0] < 2:
                    self.hideControls()
                    self.throwInWater()
                    losingSide = 1
