...
class DistributedTravelGameAI(DistributedMinigameAI):
    ...
    def gameOver(self):
        ...
        self.air.writeServerEvent('minigame_travel', self.doId, '%s|%s|%s|%s|%s|%s|%s|%s' % (ToontownGlobals.TravelGameId, self.getSafezoneId(), self.avIdList, scoreList, self.boardIndex, curVotesList, bonusesList, self.desiredNextGame))
        self.gameFSM.request('cleanup')
