...
def makeLabel(itemName, itemNum, *extraArgs):
    ...
    return DirectLabel(text = str(intVersion), text_fg = textColor, relief = DGG.RIDGE, frameSize = (-1.2, 1.2, -0.22500000000000001, 0.80000000000000004), scale = 1.0)
...
class DistributedTravelGame(DistributedMinigame):
    ...
    def loadGui(self):
        ...
        self.remainingVotesFrame = DirectFrame(parent = self.gui, relief = None, geom = DGG.getDefaultDialogGeom(), geom_color = GlobalDialogColor, geom_scale = (7, 1, 1), pos = (-0.90000000000000002, 0, 0.80000000000000004), scale = 0.10000000000000001, text = TTLocalizer.TravelGameRemainingVotes, text_align = TextNode.ALeft, text_scale = TTLocalizer.DTGremainingVotesFrame, text_pos = (-3.3999999999999999, -0.10000000000000001, 0.0))
        self.localVotesRemaining = DirectLabel(parent = self.remainingVotesFrame, relief = None, text = scoreText, text_fg = VBase4(0, 0.5, 0, 1), text_align = TextNode.ARight, text_scale = 0.69999999999999996, pos = (3.2000000000000002, 0, -0.14999999999999999))
        guiModel = loader.loadModel('phase_3.5/models/gui/friendslist_gui')
        self.choiceFrame = DirectFrame(parent = self.gui, relief = None, pos = (-0.55000000000000004, 0, -0.84999999999999998), image = DGG.getDefaultDialogGeom(), image_scale = (1.3999999999999999, 1, 0.22500000000000001), image_color = GlobalDialogColor)
        self.useLabel = DirectLabel(text = TTLocalizer.TravelGameUse, parent = self.choiceFrame, pos = (-0.58999999999999997, 0, -0.01), text_scale = TTLocalizer.DTGuseLabel, relief = None)
        self.votesPeriodLabel = DirectLabel(text = TTLocalizer.TravelGameVotesWithPeriod, parent = self.choiceFrame, pos = (-0.20999999999999999, 0, -0.01), text_scale = TTLocalizer.DTGvotesPeriodLabel, relief = None, text_align = TextNode.ALeft)
        self.votesToGoLabel = DirectLabel(text = TTLocalizer.TravelGameVotesToGo, parent = self.choiceFrame, pos = (-0.20999999999999999, 0, -0.01), text_scale = TTLocalizer.DTGvotesToGoLabel, relief = None, text_align = TextNode.ALeft)
        ...
        self.scrollList = DirectScrolledList(parent = self.choiceFrame, relief = None, pos = (-0.35999999999999999, 0, -0.02), incButton_image = (guiModel.find('**/FndsLst_ScrollUp'), guiModel.find('**/FndsLst_ScrollDN'), guiModel.find('**/FndsLst_ScrollUp_Rllvr'), guiModel.find('**/FndsLst_ScrollUp')), incButton_relief = None, incButton_pos = (0.0, 0.0, -0.040000000000000001), incButton_image3_color = Vec4(0.59999999999999998, 0.59999999999999998, 0.59999999999999998, 0.59999999999999998), incButton_scale = (1.0, 1.0, -1.0), decButton_image = (guiModel.find('**/FndsLst_ScrollUp'), guiModel.find('**/FndsLst_ScrollDN'), guiModel.find('**/FndsLst_ScrollUp_Rllvr'), guiModel.find('**/FndsLst_ScrollUp')), decButton_relief = None, decButton_pos = (0.0, 0.0, 0.095000000000000001), decButton_image3_color = Vec4(0.59999999999999998, 0.59999999999999998, 0.59999999999999998, 0.59999999999999998), itemFrame_pos = (0.0, 0.0, 0.0), itemFrame_relief = DGG.GROOVE, numItemsVisible = 1, itemMakeFunction = makeLabel, items = [], scrollSpeed = 3.0, itemFrame_scale = 0.10000000000000001, command = self.scrollChoiceChanged)
        self.putChoicesInScrollList()
        buttons = loader.loadModel('phase_3/models/gui/dialog_box_buttons_gui')
        okImageList = (buttons.find('**/ChtBx_OKBtn_UP'), buttons.find('**/ChtBx_OKBtn_DN'), buttons.find('**/ChtBx_OKBtn_Rllvr'))
        self.voteButton = DirectButton(parent = self.choiceFrame, relief = None, image = okImageList, image_scale = 3.0, pos = (0.84999999999999998, 0, 0.0), text = TTLocalizer.TravelGameVoteWithExclamation, text_scale = TTLocalizer.DTGvoteButton, text_pos = (0, 0), command = self.handleInputChoice)
        self.waitingChoicesLabel = DirectLabel(text = TTLocalizer.TravelGameWaitingChoices, text_fg = VBase4(1, 1, 1, 1), relief = None, pos = (-0.20000000000000001, 0, -0.84999999999999998), scale = 0.074999999999999997)
    ...
    def setGameReady(self):
        ...
        for index in range(self.numPlayers):
            ...
            if avatar:
                ...
                avatar.setPosHpr(-4, -4.5 + index * 3, 2.7999999999999998, 90, 0, 0)
                name = avatar.getName()
    ...
    def enterDisplayVotes(self, votes, directions, directionToGo, directionReason):
        ...
        self.dialog = TTDialog.TTDialog(text = self.resultsStr, command = self._DistributedTravelGame__cleanupDialog, style = TTDialog.NoButtons, pos = (0, 0, 1))
        self.dialog.hide()
        if self.UseTrolleyResultsPanel:
    ...
    def enterMoveTrolley(self):
        ...
        trolleyAnimateInterval = LerpFunctionInterval(self.animateTrolley, duration = self.TrolleyMoveDuration, blendType = 'easeInOut', extraArgs = [
            keyAngle,
    ...
    def enterWinMovie(self):
        ...
        self.winDialog = TTDialog.TTDialog(text = resultStr, command = self._DistributedTravelGame__cleanupWinDialog, style = TTDialog.NoButtons)
        ...
        for switch in self.switchToMinigameDict.keys():
            ...
            if labelPos:
                if useText:
                    ...
                    label = DirectLabel(text = self.idToNames[minigame], relief = None, scale = 0.10000000000000001, pos = labelPos, text_fg = (1.0, 1.0, 1.0, 1.0))
                    label.hide()
                    self.minigameLabels.append(label)
                else:
                    placeHolder = DirectButton(image = icon, relief = None, text = ('', '', self.idToNames[minigame], ''), text_scale = 0.29999999999999999, text_pos = (0, -0.69999999999999996, 0), text_fg = (1, 1, 1, 1), clickSound = None, pressEffect = 0)
            ...
            if avId == self.localAvId:
                ...
                if labelPos:
                    ...
                    bonusStr = TTLocalizer.TravelGameBonusBeans % {
                        'numBeans': beans }
                    label = DirectLabel(text = bonusStr, relief = None, scale = 0.10000000000000001, pos = labelPos, text_fg = (1.0, 1.0, 1.0, 1.0), text_align = TextNode.ALeft)
                    label.hide()
