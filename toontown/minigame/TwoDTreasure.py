...
class TwoDTreasure(DirectObject):
    ...
    def __init__(self, treasureMgr, index, pos, value, isEnemyGenerated, model):
        ...
        self.glowScalIval = Sequence(LerpScaleInterval(self.glowCard, 0.40000000000000002, scale = 4.0999999999999996, startScale = 4.9000000000000004), LerpScaleInterval(self.glowCard, 0.40000000000000002, scale = 4.9000000000000004, startScale = 4.0999999999999996))
        self.glowScalIval.loop()
        self.modelScalIval = Sequence(LerpScaleInterval(self.model, 0.40000000000000002, scale = 1.0, startScale = 1.03), LerpScaleInterval(self.model, 0.40000000000000002, scale = 1.03, startScale = 1.0))
        ...
        self.accept('enter' + self.sphereName, self._TwoDTreasure__handleEnterSphere)
        ...
        if self.isEnemyGenerated:
            ...
            self.flash.setPos(self.model.getX(), self.model.getY() - 0.20000000000000001, self.model.getZ())
    ...
    def popupEnemyTreasure(self):
        ...
        self.appearEffect = Sequence(Wait(2.3999999999999999), Func(self.showTreasure), Parallel(modelFadeIn, flashFadeOut, Func(base.playSfx, self.game.assetMgr.sparkleSound)))
