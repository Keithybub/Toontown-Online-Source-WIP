...
class DistributedLawbotBoss(DistributedBossCog.DistributedBossCog, FSM.FSM):
    ...
    def setLawyerIds(self, lawyerIds):
        self.lawyers = []
        self.cr.relatedObjectMgr.abortRequest(self.lawyerRequest)
        self.lawyerRequest = self.cr.relatedObjectMgr.requestObjects(lawyerIds, allCallback = self._DistributedLawbotBoss__gotLawyers)
    ...
    def setBossDamage(self, bossDamage, recoverRate, timestamp):
        ...
        if self.bossDamageMovie:
            if self.bossDamage >= self.bossMaxDamage:
                ...
                if self.recoverRate:
                    taskMgr.add(self._DistributedLawbotBoss__recoverBossDamage, taskName)
    ...
    def _DistributedLawbotBoss__walkToonToPromotion(self, toonId, delay, mopath, track, delayDeletes):
    ...
    def _DistributedLawbotBoss__showWaitingMessage(self, task):
        ...
        self._DistributedLawbotBoss__showOnscreenMessage(TTLocalizer.BuildingWaitingForVictors)
    ...
    def loadScaleOld(self):
        ...
        self.defensePanNodePath.reparentTo(self.beamNodePath)
        defenseTube = CollisionTube(0, 0, -0.5, 0, 0, -1.5, 0.59999999999999998)
        ...
        self.prosecutionPanNodePath.reparentTo(self.beamNodePath)
        prosecutionTube = CollisionTube(0, 0, -0.5, 0, 0, -1.5, 0.59999999999999998)
    ...
    def enterRollToBattleTwo(self):
        ...
        seq = Sequence(self._DistributedLawbotBoss__makeRollToBattleTwoMovie(), Func(self._DistributedLawbotBoss__onToPrepareBattleTwo), name = intervalName)
        seq.start()
    ...
    def enterPrepareBattleTwo(self):
        ...
        seq.start()
        self.storeInterval(seq, intervalName)
        self.acceptOnce('doneChatPage', self._DistributedLawbotBoss__showCannonsAppearing)
        base.playMusic(self.stingMusic, looping = 0, volume = 1.0)

    
    def _DistributedLawbotBoss__showCannonsAppearing(self, elapsedTime = 0):
        allCannonsAppear = Sequence(Func(self._DistributedLawbotBoss__positionToonsInFrontOfCannons), Func(camera.reparentTo, localAvatar), Func(camera.setPos, localAvatar.cameraPositions[2][0]), Func(camera.lookAt, localAvatar))
        ...
        for toonId in self.involvedToons:
            ...
            self.cannons.has_key(index)
        
        allCannonsAppear.append(multiCannons)
        intervalName = 'prepareBattleTwoCannonsAppear'
        seq = Sequence(allCannonsAppear, Func(self._DistributedLawbotBoss__onToBattleTwo), name = intervalName)
        seq.start()
        self.storeInterval(seq, intervalName)
    ...
    def _DistributedLawbotBoss__onToBattleTwo(self, elapsedTime = 0):
        ...
        taskMgr.doMethodLater(1, self._DistributedLawbotBoss__showWaitingMessage, self.uniqueName('WaitingMessage'))
    ...
    def enterRollToBattleThree(self):
        ...
        seq = Sequence(self._DistributedLawbotBoss__makeRollToBattleThreeMovie(), Func(self._DistributedLawbotBoss__onToPrepareBattleThree), name = intervalName)
        seq.start()
        self.storeInterval(seq, intervalName)
        base.playMusic(self.betweenBattleMusic, looping = 1, volume = 0.90000000000000002)
    ...
    def enterPrepareBattleThree(self):
        ...
        self.acceptOnce('doneChatPage', self._DistributedLawbotBoss__onToBattleThree)
    ...
    def enterBattleThree(self):
        ...
        self.forward = 1
        self.doAnimate()
        self.accept('enterWitnessStand', self._DistributedLawbotBoss__touchedWitnessStand)
        self.accept('pieSplat', self._DistributedLawbotBoss__pieSplat)
        self.accept('localPieSplat', self._DistributedLawbotBoss__localPieSplat)
        ...
        self.accept('enterDefenseCol', self._DistributedLawbotBoss__enterDefenseCol)
        self.accept('enterProsecutionCol', self._DistributedLawbotBoss__enterProsecutionCol)
        localAvatar.setCameraFov(ToontownGlobals.BossBattleCameraFov)
        taskMgr.doMethodLater(30, self._DistributedLawbotBoss__howToGetPies, self.uniqueName('PieAdvice'))
        self.stickBossToFloor()
        self.setPosHpr(*ToontownGlobals.LawbotBossBattleThreePosHpr)
        self.bossMaxDamage = ToontownGlobals.LawbotBossMaxDamage
    ...
    def enterVictory(self):
        ...
        seq = Sequence(self.makeVictoryMovie(), Func(self._DistributedLawbotBoss__continueVictory), name = intervalName)
        seq.start()
        self.storeInterval(seq, intervalName)
        base.playMusic(self.battleThreeMusic, looping = 1, volume = 0.90000000000000002, time = self.battleThreeMusicTime)
    ...
    def enterDefeat(self):
        ...
        seq = Sequence(self.makeDefeatMovie(), Func(self._DistributedLawbotBoss__continueDefeat), name = intervalName)
        seq.start()
        self.storeInterval(seq, intervalName)
        base.playMusic(self.battleThreeMusic, looping = 1, volume = 0.90000000000000002, time = self.battleThreeMusicTime)
    ...
    def enterReward(self):
        ...
        ival = Sequence(Parallel(victory, camVictory), Func(self._DistributedLawbotBoss__doneReward))
    ...
    def _DistributedLawbotBoss__toonsToPromotionPosition(self, toonIds, battleNode):
    ...
    def _DistributedLawbotBoss__outOfPies(self):
        ...
        self._DistributedLawbotBoss__showOnscreenMessage(TTLocalizer.LawbotBossNeedMoreEvidence)
        taskMgr.doMethodLater(20, self._DistributedLawbotBoss__howToGetPies, self.uniqueName('PieAdvice'))

    
    def _DistributedLawbotBoss__howToGetPies(self, task):
        self.notify.debug('----- __howToGetPies')
        self._DistributedLawbotBoss__showOnscreenMessage(TTLocalizer.LawbotBossHowToGetEvidence)

    
    def _DistributedLawbotBoss__howToThrowPies(self, task):
        self.notify.debug('----- __howToThrowPies')
        self._DistributedLawbotBoss__showOnscreenMessage(TTLocalizer.LawbotBossHowToThrowPies)
    ...
    def _DistributedLawbotBoss__touchedWitnessStand(self, entry):
        ...
        if not self.everThrownPie:
            taskMgr.doMethodLater(30, self._DistributedLawbotBoss__howToThrowPies, self.uniqueName('PieAdvice'))
    ...
    def makeVictoryMovie(self):
        ...
        bossTrack = Track((0.5, Sequence(Func(self.clearChat), Func(camera.reparentTo, render), Func(camera.setPos, -3, 45, 25), Func(camera.setHpr, 0, 10, 0))), (1.0, Func(self.setChatAbsolute, TTLocalizer.LawbotBossDefenseWins1, CFSpeech)), (5.5, Func(self.setChatAbsolute, TTLocalizer.LawbotBossDefenseWins2, CFSpeech)), (9.5, Sequence(Func(camera.wrtReparentTo, render))), (9.5999999999999996, Parallel(rollTrack, Func(self.setChatAbsolute, TTLocalizer.LawbotBossDefenseWins3, CFSpeech), self.door3.posInterval(2, doorEndPos, startPos = doorStartPos))), (13.1, Sequence(self.door3.posInterval(1, doorStartPos))))
    ...
    def _DistributedLawbotBoss__makePrepareBattleThreeMovie(self):
        movie = Sequence(Func(camera.reparentTo, render), Func(camera.setPos, -15, 15, 20), Func(camera.setHpr, -90, 0, 0), Wait(3), Func(camera.reparentTo, self.witnessToon), Func(camera.setPos, 0, 8, 2), Func(camera.setHpr, 180, 10, 0), Func(self._DistributedLawbotBoss__doWitnessPrepareBattleThreeChat))
        return movie
    ...
    def flashPanBlue(self):
        ...
        seq = Sequence(self.defensePanNodePath.colorScaleInterval(0.10000000000000001, colorScale = VBase4(0, 0, 1, 1)), self.defensePanNodePath.colorScaleInterval(0.29999999999999999, colorScale = VBase4(1, 1, 1, 1)), name = intervalName)
