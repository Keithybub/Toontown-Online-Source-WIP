...
class DistributedBossbotBoss(DistributedBossCog.DistributedBossCog, FSM.FSM):
    ...
    def announceGenerate(self):
        ...
        overtimeOneClubSequence = Sequence(self.bossClub.colorScaleInterval(0.10000000000000001, colorScale = VBase4(0, 1, 0, 1)), self.bossClub.colorScaleInterval(0.29999999999999999, colorScale = VBase4(1, 1, 1, 1)))
        overtimeTwoClubSequence = Sequence(self.bossClub.colorScaleInterval(0.10000000000000001, colorScale = VBase4(1, 0, 0, 1)), self.bossClub.colorScaleInterval(0.29999999999999999, colorScale = VBase4(1, 1, 1, 1)))
    ...
    def enterPrepareBattleTwo(self):
        ...
        delayDeletes = []
        seq = Sequence(self.makePrepareBattleTwoMovie(delayDeletes), Func(self._DistributedBossbotBoss__onToBattleTwo), name = intervalName)
        seq.delayDeletes = delayDeletes
        seq.start()
        self.storeInterval(seq, intervalName)
        base.playMusic(self.betweenPhaseMusic, looping = 1, volume = 0.90000000000000002)
    ...
    def enterBattleTwo(self):
        ...
        base.playMusic(self.phaseTwoMusic, looping = 1, volume = 0.90000000000000002)
    ...
    def enterPrepareBattleThree(self):
        ...
        seq = Sequence(self.makePrepareBattleThreeMovie(), Func(self._DistributedBossbotBoss__onToBattleThree), name = intervalName)
        seq.start()
        self.storeInterval(seq, intervalName)
        base.playMusic(self.betweenPhaseMusic, looping = 1, volume = 0.90000000000000002)
    ...
    def enterPrepareBattleFour(self):
        ...
        seq = Sequence(self.makePrepareBattleFourMovie(), Func(self._DistributedBossbotBoss__onToBattleFour), name = intervalName)
        seq.start()
        self.storeInterval(seq, intervalName)
        base.playMusic(self.phaseFourMusic, looping = 1, volume = 0.90000000000000002)
    ...
    def makePrepareBattleFourMovie(self):
        ...
        track = Sequence(Func(self._DistributedBossbotBoss__showResistanceToon, True), Func(rToon.setPos, Point3(0, -5, 0)), Func(rToon.setHpr, Point3(0, 0, 0)), Func(camera.reparentTo, rToon), Func(camera.setPos, Point3(0, 13, 3 + offsetZ)), Func(camera.setHpr, Point3(-180, 0, 0)), Func(self.banquetDoor.setH, 90), Func(rToon.setChatAbsolute, TTL.BossbotRTPhase4Speech1, CFSpeech), Wait(4.0), Func(rToon.setChatAbsolute, TTL.BossbotRTPhase4Speech2, CFSpeech), Wait(4.0), Func(self._DistributedBossbotBoss__hideResistanceToon), Func(camera.reparentTo, self), Func(camera.setPos, Point3(0, -45, 5)), Func(camera.setHpr, Point3(0, 14, 0)), Func(self.setChatAbsolute, TTL.BossbotPhase4Speech1, CFSpeech), Func(self.banquetDoor.setH, 0), Wait(3.0), Func(self.setChatAbsolute, TTL.BossbotPhase4Speech2, CFSpeech), Func(self.bossClub.setScale, 0.01), Func(self.bossClub.reparentTo, self.rightHandJoint), LerpScaleInterval(self.bossClub, 3, Point3(1, 1, 1)), Func(self.clearChat))
        return track
    ...
    def enterVictory(self):
        ...
        seq = Sequence(self.makeVictoryMovie(), Func(self._DistributedBossbotBoss__continueVictory), name = intervalName)
        seq.start()
        self.storeInterval(seq, intervalName)
        base.playMusic(self.phaseFourMusic, looping = 1, volume = 0.90000000000000002)
    ...
    def enterReward(self):
        ...
        ival = Sequence(Parallel(victory, camVictory), Func(self._DistributedBossbotBoss__doneReward))
    ...
    def _DistributedBossbotBoss__arrangeToonsAroundResistanceToonForReward(self):
        radius = 7
        numToons = len(self.involvedToons)
        center = (numToons - 1) / 2.0
        for i in range(numToons):
        ...
        if toon:
            ...
            for i in range(4):
                ...
                def detachNodeLater(node = node):
                    ...
                    self.doMethodLater(0.0050000000000000001, detachNode, 'detach-%s-%s' % (gearRoot.getName(), node.getName()), extraArgs = [
                        node])

                gearTrack.append(Sequence(Wait(i * 0.14999999999999999), Func(node.show), Parallel(node.posInterval(1, Point3(x, distance, z), fluid = 1), node.hprInterval(1, VBase3(h, 0, 0), fluid = 1)), Func(detachNodeLater)))
            ...
            seq = Sequence(ParallelEndTogether(self.pelvis.hprInterval(1, VBase3(toToonH, 0, 0)), neutral1Anim), extraAnim, Parallel(Sequence(Wait(0.19), gearTrack, Func(detachGearRootLater), self.pelvis.hprInterval(0.20000000000000001, VBase3(0, 0, 0))), Sequence(throwAnim, neutral2Anim)))
    ...
    def setSpeedDamage(self, speedDamage, recoverRate, timestamp):
        ...
        if self.speedRecoverRate:
            taskMgr.add(self._DistributedBossbotBoss__recoverSpeedDamage, taskName)
    ...
    def doGolfAttack(self, avId, attackCode):
        ...
        if toon:
            ...
            for i in range(5):
                ...
                gearTrack.append(Sequence(Wait(26.0 / 24.0), Wait(i * 0.14999999999999999), Func(node.show), Parallel(node.posInterval(1, Point3(x, y, z), fluid = 1), node.hprInterval(1, VBase3(0, p, 0), fluid = 1)), Func(detachNodeLater)))
            ...
            seq = Sequence(ParallelEndTogether(self.pelvis.hprInterval(1, VBase3(toToonH, 0, 0)), neutral1Anim), extraAnim, Parallel(Sequence(Wait(0.19), gearTrack, Func(detachGearRootLater), self.pelvis.hprInterval(0.20000000000000001, VBase3(0, 0, 0))), Sequence(throwAnim, neutral2Anim), Sequence(Wait(0.84999999999999998), SoundInterval(self.swingClubSfx, node = self, duration = 0.45000000000000001, cutOff = 300, listenerNode = base.localAvatar))))
            self.doAnimate(seq, now = 1, raised = 1)
    ...
    def doGolfAreaAttack(self):
        ...
        seq = Sequence(Func(base.playSfx, self.warningSfx), Func(self.saySomething, TTLocalizer.GolfAreaAttackTaunt), ParallelEndTogether(self.pelvis.hprInterval(2, VBase3(toToonH, 0, 0)), neutral1Anim), extraAnim, Parallel(Sequence(rotateFire, Func(detachGearRoots), Func(self.pelvis.setHpr, VBase3(0, 0, 0))), Sequence(throwAnim, neutral2Anim), Sequence(Wait(0.84999999999999998), SoundInterval(self.swingClubSfx, node = self, duration = 0.45000000000000001, cutOff = 300, listenerNode = base.localAvatar))))
        self.doAnimate(seq, now = 1, raised = 1)
    ...
    def doOvertimeAttack(self, index):
        ...
        if index < len(self.belts):
            ...
            for i in range(4):
                ...
                z = random.uniform(-3, 3)
                h = random.uniform(-720, 720)
                gearTrack.append(Sequence(Wait(i * 0.14999999999999999), Func(node.show), Parallel(node.posInterval(1, Point3(x, distance, z), fluid = 1), node.hprInterval(1, VBase3(h, 0, 0), fluid = 1)), Func(node.detachNode)))
            ...
            seq = Sequence(ParallelEndTogether(self.pelvis.hprInterval(1, VBase3(toToonH, 0, 0)), neutral1Anim), extraAnim, Parallel(Sequence(Wait(0.19), gearTrack, Func(gearRoot.detachNode), Func(self.explodeSfx.play), self.pelvis.hprInterval(0.20000000000000001, VBase3(0, 0, 0))), Sequence(throwAnim, neutral2Anim)), Func(belt.request, 'Inactive'))
