...
class BossCog(Avatar.Avatar):
    ...
    def updateHealthBar(self):
        ...
        if self.healthCondition != condition:
            if condition == 4:
                blinkTask = Task.loop(Task(self._BossCog__blinkRed), Task.pause(0.75), Task(self._BossCog__blinkGray), Task.pause(0.10000000000000001))
                taskMgr.add(blinkTask, self.uniqueName('blink-task'))
            elif condition == 5:
                if self.healthCondition == 4:
                    taskMgr.remove(self.uniqueName('blink-task'))
                
                blinkTask = Task.loop(Task(self._BossCog__blinkRed), Task.pause(0.25), Task(self._BossCog__blinkGray), Task.pause(0.10000000000000001))
                taskMgr.add(blinkTask, self.uniqueName('blink-task'))
            else:
    ...
    def getAnim(self, anim):
        ...
        if anim == None:
            ...
            ival = Sequence(Track((0, ival), (0, SoundInterval(self.spinSfx, node = self)), (0.90000000000000002, Parallel(SoundInterval(self.rainGearsSfx, node = self), ParticleInterval(pe, self.frontAttack, worldRelative = 0, duration = 1.5, cleanup = True), duration = 0)), (1.8999999999999999, Func(self.bubbleF.unstash))), Func(self.bubbleF.stash))
            self.forward = 1
            self.happy = 0
            self.raised = 1
