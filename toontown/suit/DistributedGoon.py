...
class DistributedGoon(DistributedCrushableEntity.DistributedCrushableEntity, Goon.Goon, FSM.FSM):
    ...
    def announceGenerate(self):
        ...
        if self.level:
            ...
            self.enterOff()
            taskMgr.doMethodLater(0.10000000000000001, self.makeCollidable, self.taskName('makeCollidable'))
    ...
    def playCrushMovie(self, crusherId, axis):
        goonPos = self.getPos()
        sx = random.uniform(0.29999999999999999, 0.80000000000000004) * self.scale
        sz = random.uniform(0.29999999999999999, 0.80000000000000004) * self.scale
