...
class DistributedLawbotBossSuit(DistributedSuitBase.DistributedSuitBase):
    ...
    def enterWalk(self):
        ...
        self.enableBattleDetect('walk', self._DistributedLawbotBossSuit__handleToonCollision)
        self.loop('walk', 0)
        pathPoints = [
            Vec3(50, 15, 0),
            Vec3(50, 25, 0),
    ...
    def makeProsecuteThrowingTrack(self, evidence, inFlightDuration, hitPos):
        ...
        propTrack = Sequence(Func(evidence.hide), Func(evidence.setPos, 0, 0.5, -0.29999999999999999), Func(evidence.reparentTo, self.getRightHand()), Wait(self.timeToShow), Func(evidence.show), Wait(self.timeToRelease - self.timeToShow), Func(evidence.wrtReparentTo, render), Func(self.makeDummySequence), inFlight, Func(evidence.detachNode))
    ...
    def makeAttackThrowingTrack(self, evidence, inFlightDuration, hitPos):
        ...
        self.setHpr(origHpr)
        rotateTrack = Sequence(self.hprInterval(self.timeToShow, newHpr, fluid = 1))
        propTrack = Sequence(Func(evidence.hide), Func(evidence.setPos, 0, 0.5, -0.29999999999999999), Func(evidence.reparentTo, self.getRightHand()), Wait(self.timeToShow), Func(evidence.show), Wait(self.timeToRelease - self.timeToShow), Func(evidence.wrtReparentTo, render), Func(evidence.setZ, 1.3), evidence.posInterval(inFlightDuration, hitPos, fluid = 1), Func(evidence.detachNode))
        throwingTrack = Parallel(suitTrack, propTrack, rotateTrack)
        return throwingTrack
    ...
    def makePreThrowAttackTrack(self, evidence, inFlightDuration, hitPos):
        ...
        self.setHpr(origHpr)
        rotateTrack = Sequence(self.hprInterval(self.timeToShow, newHpr, fluid = 1))
        propTrack = Sequence(Func(evidence.hide), Func(evidence.setPos, 0, 0.5, -0.29999999999999999), Func(evidence.setScale, 1), Func(evidence.setHpr, 0, 0, 0), Func(evidence.reparentTo, self.getRightHand()), Wait(self.timeToShow), Func(evidence.show), Wait(self.timeToRelease - self.timeToShow))
        throwingTrack = Parallel(suitTrack, propTrack, rotateTrack)
        return throwingTrack
