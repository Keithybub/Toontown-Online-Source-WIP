...
class DistributedLawbotCannon(DistributedObject.DistributedObject):
    ...
    def activateCannons(self):
        if not self.cannonsActive:
            ...
            self.accept(self.uniqueName('enterCannonSphere'), self._DistributedLawbotCannon__handleEnterSphere)
    ...
    def setMovie(self, mode, avId, extraInfo):
        ...
        if mode == CannonGlobals.CANNON_MOVIE_CLEAR:
            ...
            if self.cr.doId2do.has_key(self.avId):
                ...
                self.acceptOnce(self.av.uniqueName('disable'), self._DistributedLawbotCannon__avatarGone)
                self.av.loop('neutral')
                self.av.stopSmooth()
    ...
    def _DistributedLawbotCannon__makeGui(self):
        ...
        self.fireButton = DirectButton(parent = self.aimPad, image = ((guiModel, '**/Fire_Btn_UP'), (guiModel, '**/Fire_Btn_DN'), (guiModel, '**/Fire_Btn_RLVR')), relief = None, pos = (0.0115741, 0, 0.0050505100000000002), scale = 1.0, command = self._DistributedLawbotCannon__firePressed)
        ...
        self.cannonBallLabel = DirectLabel(parent = self.aimPad, text = cannonBallText, text_fg = VBase4(1, 1, 1, 1), text_align = TextNode.ACenter, relief = None, pos = (0.47499999999999998, 0.0, -0.34999999999999998), scale = 0.25)
        ...
        bindButton(self.upButton, self._DistributedLawbotCannon__upPressed, self._DistributedLawbotCannon__upReleased)
        bindButton(self.downButton, self._DistributedLawbotCannon__downPressed, self._DistributedLawbotCannon__downReleased)
        bindButton(self.leftButton, self._DistributedLawbotCannon__leftPressed, self._DistributedLawbotCannon__leftReleased)
        bindButton(self.rightButton, self._DistributedLawbotCannon__rightPressed, self._DistributedLawbotCannon__rightReleased)
    ...
    def _DistributedLawbotCannon__enableAimInterface(self):
        self.aimPad.show()
        self.accept(self.FIRE_KEY, self._DistributedLawbotCannon__fireKeyPressed)
        self.accept(self.UP_KEY, self._DistributedLawbotCannon__upKeyPressed)
        self.accept(self.DOWN_KEY, self._DistributedLawbotCannon__downKeyPressed)
        self.accept(self.LEFT_KEY, self._DistributedLawbotCannon__leftKeyPressed)
        self.accept(self.RIGHT_KEY, self._DistributedLawbotCannon__rightKeyPressed)
    ...
    def _DistributedLawbotCannon__fireKeyPressed(self):
        self.ignore(self.FIRE_KEY)
        self.accept(self.FIRE_KEY + '-up', self._DistributedLawbotCannon__fireKeyReleased)
        self._DistributedLawbotCannon__firePressed()

    
    def _DistributedLawbotCannon__upKeyPressed(self):
        self.ignore(self.UP_KEY)
        self.accept(self.UP_KEY + '-up', self._DistributedLawbotCannon__upKeyReleased)
        self._DistributedLawbotCannon__upPressed()

    
    def _DistributedLawbotCannon__downKeyPressed(self):
        self.ignore(self.DOWN_KEY)
        self.accept(self.DOWN_KEY + '-up', self._DistributedLawbotCannon__downKeyReleased)
        self._DistributedLawbotCannon__downPressed()

    
    def _DistributedLawbotCannon__leftKeyPressed(self):
        self.ignore(self.LEFT_KEY)
        self.accept(self.LEFT_KEY + '-up', self._DistributedLawbotCannon__leftKeyReleased)
        self._DistributedLawbotCannon__leftPressed()

    
    def _DistributedLawbotCannon__rightKeyPressed(self):
        self.ignore(self.RIGHT_KEY)
        self.accept(self.RIGHT_KEY + '-up', self._DistributedLawbotCannon__rightKeyReleased)
    ...
    def _DistributedLawbotCannon__upPressed(self):
        ...
        self.upPressed = self._DistributedLawbotCannon__enterControlActive(self.upPressed)

    
    def _DistributedLawbotCannon__downPressed(self):
        self.notify.debug('down pressed')
        self.downPressed = self._DistributedLawbotCannon__enterControlActive(self.downPressed)

    
    def _DistributedLawbotCannon__leftPressed(self):
        self.notify.debug('left pressed')
        self.leftPressed = self._DistributedLawbotCannon__enterControlActive(self.leftPressed)

    
    def _DistributedLawbotCannon__rightPressed(self):
        self.notify.debug('right pressed')
        self.rightPressed = self._DistributedLawbotCannon__enterControlActive(self.rightPressed)

    
    def _DistributedLawbotCannon__upReleased(self):
        self.notify.debug('up released')
        self.upPressed = self._DistributedLawbotCannon__exitControlActive(self.upPressed)

    
    def _DistributedLawbotCannon__downReleased(self):
        self.notify.debug('down released')
        self.downPressed = self._DistributedLawbotCannon__exitControlActive(self.downPressed)

    
    def _DistributedLawbotCannon__leftReleased(self):
        self.notify.debug('left released')
        self.leftPressed = self._DistributedLawbotCannon__exitControlActive(self.leftPressed)

    
    def _DistributedLawbotCannon__rightReleased(self):
        self.notify.debug('right released')
        self.rightPressed = self._DistributedLawbotCannon__exitControlActive(self.rightPressed)
    ...
    def _DistributedLawbotCannon__fireCannonTask(self, task):
        ...
        flightResults = self._DistributedLawbotCannon__calcFlightResults(avId, launchTime)
        if not isClient():
        ...
        self.accept(self.uniqueName('cannonHit'), self._DistributedLawbotCannon__handleCannonHit)
        shootTask = Task(self._DistributedLawbotCannon__shootTask, self.taskName('shootTask'))
        flyTask = Task(self._DistributedLawbotCannon__flyTask, self.taskName('flyTask'))
        shootTask.info = info
        flyTask.info = info
        seqTask = Task.sequence(shootTask, flyTask)
        taskMgr.add(seqTask, self.taskName('flyingToon') + '-' + str(avId))
        self.acceptOnce(self.uniqueName('stopFlyTask'), self._DistributedLawbotCannon__stopFlyTask)
        return Task.done
    ...
    def _DistributedLawbotCannon__handleCannonHit(self, collisionEntry):
        ...
        if hitNode in chairlist:
            track.append(Func(self._DistributedLawbotCannon__hitChair, self.jurorToon, pos))
            track.append(Wait(1.0))
            track.append(Func(self._DistributedLawbotCannon__setToonUpright, self.av))
            if self.av == base.localAvatar:
                strs = hitNode.split('-')
                chairNum = int(strs[1])
            ...
            track.append(Func(self._DistributedLawbotCannon__hitGround, self.jurorToon, pos))
            track.append(Wait(1.0))
            track.append(Func(self._DistributedLawbotCannon__setToonUpright, self.av))
    ...
    def generateCannonAppearTrack(self, avatar):
        ...
        self.cannon.show()
        kartTrack = Parallel(Sequence(ActorInterval(avatar, 'feedPet'), Func(avatar.loop, 'neutral')), Sequence(Func(self.cannon.reparentTo, avatar.rightHand), Wait(2.1000000000000001), Func(self.cannon.wrtReparentTo, render), Func(self.cannon.setShear, 0, 0, 0), Parallel(LerpHprInterval(self.cannon, hpr = self.nodePath.getHpr(render), duration = 1.2), ProjectileInterval(self.cannon, endPos = self.nodePath.getPos(render), duration = 1.2, gravityMult = 0.45000000000000001)), Wait(0.20000000000000001), Sequence(LerpScaleInterval(self.cannon, scale = Point3(1.1000000000000001, 1.1000000000000001, 0.10000000000000001), duration = 0.20000000000000001), LerpScaleInterval(self.cannon, scale = Point3(0.90000000000000002, 0.90000000000000002, 0.10000000000000001), duration = 0.10000000000000001), LerpScaleInterval(self.cannon, scale = Point3(1.0, 1.0, 0.10000000000000001), duration = 0.10000000000000001), LerpScaleInterval(self.cannon, scale = Point3(1.0, 1.0, 1.1000000000000001), duration = 0.20000000000000001), LerpScaleInterval(self.cannon, scale = Point3(1.0, 1.0, 0.90000000000000002), duration = 0.10000000000000001), LerpScaleInterval(self.cannon, scale = Point3(1.0, 1.0, 1.0), duration = 0.10000000000000001), Func(self.cannon.wrtReparentTo, self.nodePath))))
        return kartTrack
