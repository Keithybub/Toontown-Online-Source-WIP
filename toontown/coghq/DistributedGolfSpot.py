...
class DistributedGolfSpot(DistributedObject.DistributedObject, FSM.FSM):
    ...
    positions = ((-45, 100, GolfGlobals.GOLF_BALL_RADIUS), (-15, 100, GolfGlobals.GOLF_BALL_RADIUS), (15, 100, GolfGlobals.GOLF_BALL_RADIUS), (45, 100, GolfGlobals.GOLF_BALL_RADIUS))
    ...
    def announceGenerate(self):
        ...
        self.loadAssets()
        self.accept('flyBallHit-%d' % self.index, self._DistributedGolfSpot__flyBallHit)
    ...
    def loadAssets(self):
        ...
        self.clubLookatSpot.setY(-(GolfGlobals.GOLF_BALL_RADIUS + 0.10000000000000001))
    ...
    def enterFree(self):
        ...
        if self.avId == localAvatar.doId:
            if not self.isDisabled():
                ...
                taskMgr.doMethodLater(5, self._DistributedGolfSpot__allowDetect, self.triggerName)
                self.fadeTrack = Sequence(Func(self.ballModel.setTransparency, 1), self.ballModel.colorScaleInterval(0.20000000000000001, VBase4(1, 1, 1, 0.29999999999999999)), name = 'fadeTrack-enterFree')
    ...
    def _DistributedGolfSpot__allowDetect(self, task):
        ...
        self.fadeTrack = Sequence(self.ballModel.colorScaleInterval(0.20000000000000001, self.ballColor), Func(self.ballModel.clearTransparency), name = 'fadeTrack-allowDetect')
        self.fadeTrack.start()
        self.trigger.unstash()
        self.accept(self.triggerEvent, self._DistributedGolfSpot__hitTrigger)
    ...
    def _DistributedGolfSpot__enableControlInterface(self):
        ...
        self.closeButton = DirectButton(image = (gui.find('**/CloseBtn_UP'), gui.find('**/CloseBtn_DN'), gui.find('**/CloseBtn_Rllvr'), gui.find('**/CloseBtn_UP')), relief = None, scale = 2, text = TTLocalizer.BossbotGolfSpotLeave, text_scale = 0.040000000000000001, text_pos = (0, -0.070000000000000007), text_fg = VBase4(1, 1, 1, 1), pos = (1.05, 0, -0.81999999999999995), command = self._DistributedGolfSpot__exitGolfSpot)
        ...
        self.accept('InputState-turnLeft', self._DistributedGolfSpot__leftArrow)
        self.accept('InputState-turnRight', self._DistributedGolfSpot__rightArrow)
        taskMgr.add(self._DistributedGolfSpot__watchControls, 'watchGolfSpotControls')
        taskMgr.doMethodLater(5, self._DistributedGolfSpot__displayGolfSpotAdvice, self.golfSpotAdviceName)
        self.arrowVert = 0
        self.arrowHorz = 0
        if self.powerBar:
            self.powerBar.show()
    ...
    def setupPowerBar(self):
        self.powerBar = DirectWaitBar(pos = (0.0, 0, -0.93999999999999995), relief = DGG.SUNKEN, frameSize = (-2.0, 2.0, -0.20000000000000001, 0.20000000000000001), borderWidth = (0.02, 0.02), scale = 0.25, range = 100, sortOrder = 50, frameColor = (0.5, 0.5, 0.5, 0.5), barColor = (1.0, 0.0, 0.0, 1.0), text = '', text_scale = 0.26000000000000001, text_fg = (1, 1, 1, 1), text_align = TextNode.ACenter, text_pos = (0, -0.050000000000000003))
        self.power = 0
    ...
    def _DistributedGolfSpot__displayGolfSpotAdvice(self, task):
        if self.golfSpotAdviceLabel == None:
            self.golfSpotAdviceLabel = DirectLabel(text = TTLocalizer.BossbotGolfSpotAdvice, text_fg = VBase4(1, 1, 1, 1), text_align = TextNode.ACenter, relief = None, pos = (0, 0, 0.68999999999999995), scale = 0.10000000000000001)
    ...
    def showExiting(self):
        if self.closeButton:
            self.closeButton.destroy()
            self.closeButton = DirectLabel(relief = None, text = TTLocalizer.BossbotGolfSpotLeaving, pos = (1.05, 0, -0.88), text_pos = (0, 0), text_scale = 0.059999999999999998, text_fg = VBase4(1, 1, 1, 1))
    ...
    def _DistributedGolfSpot__beginFireBall(self):
        ...
        taskMgr.add(self._DistributedGolfSpot__updateBallPower, self.ballPowerTaskName)
    ...
    def _DistributedGolfSpot__endFireBall(self):
        ...
        self._DistributedGolfSpot__flyBallSequenceNum = (self._DistributedGolfSpot__flyBallSequenceNum + 1) % 255
        self.sendSwingInfo(power, angle, self._DistributedGolfSpot__flyBallSequenceNum)
        self.setSwingInfo(power, angle, self._DistributedGolfSpot__flyBallSequenceNum)
        self.resetPowerBar()
    ...
    def _DistributedGolfSpot__updateBallPower(self, task):
        ...
        newPower = self._DistributedGolfSpot__getBallPower(globalClock.getFrameTime())
        self.power = newPower
    ...
    def startPosHprBroadcast(self):
        ...
        taskMgr.remove(taskName)
        taskMgr.doMethodLater(self._DistributedGolfSpot__broadcastPeriod, self._DistributedGolfSpot__posHprBroadcast, taskName)
    ...
    def getFlyBallBubble(self):
        if self._DistributedGolfSpot__flyBallBubble == None:
