...
class DistributedCashbotBossObject(DistributedSmoothNode.DistributedSmoothNode, FSM.FSM):
    ...
    def __init__(self, cr):
        ...
        self.touchedBossSoundInterval = SoundInterval(self.touchedBossSfx, duration = 0.80000000000000004)
    ...
    def activatePhysics(self):
        if not self.physicsActivated:
            ...
            self.accept(self.collideName + '-floor', self._DistributedCashbotBossObject__hitFloor)
            self.accept(self.collideName + '-goon', self._DistributedCashbotBossObject__hitGoon)
            self.acceptOnce(self.collideName + '-headTarget', self._DistributedCashbotBossObject__hitBoss)
            self.accept(self.collideName + '-dropPlane', self._DistributedCashbotBossObject__hitDropPlane)
    ...
    def enterSlidingFloor(self, avId):
        ...
        if self.avId == base.localAvatar.doId:
            ...
            if self.wantsWatchDrift:
                taskMgr.add(self._DistributedCashbotBossObject__watchDrift, self.watchDriftName)
