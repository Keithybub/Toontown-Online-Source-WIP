...
class DistributedCrate(DistributedCrushableEntity.DistributedCrushableEntity):
    ...
    def handleCollision(self, collEntry = None):
        ...
        if relativeVec.getY() < -0.69999999999999996 and offsetDot > 0.90000000000000002 and offsetVec.getZ() < 0.050000000000000003:
            ...
            self._DistributedCrate__startStickTask(crateNormal, base.localAvatar.getPos(render))
    ...
    def _DistributedCrate__stickTask(self, task):
        ...
        if tElapsed > self.tStick:
            ...
            startHpr = Vec3(h, 0, 0)
            self.avPushTrack = Sequence(LerpPosHprInterval(lToon, 0.25, newPos, newHpr, startHpr = startHpr, other = self, blendType = 'easeInOut'), Func(place.fsm.request, 'push'), Func(self._DistributedCrate__sendPushRequest, task.crateNormal), SoundInterval(self.creakSound, node = self))
