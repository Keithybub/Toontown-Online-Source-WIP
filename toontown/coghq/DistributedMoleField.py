...
class DistributedMoleField(DistributedNodePathEntity, MoleFieldBase.MoleFieldBase):
    ...
    def announceGenerate(self):
        ...
        taskMgr.doMethodLater(0.10000000000000001, self._DistributedMoleField__detect, self.detectName)
        self.calcDimensions()
    ...
    def loadGui(self):
        ...
        self.scoreLabel = DirectLabel(parent = self.frame2D, relief = None, pos = (0, 0, 0), scale = 1.0, text = '', text_font = ToontownGlobals.getSignFont(), text0_fg = (1, 1, 1, 1), text_scale = 0.074999999999999997, text_pos = (0, -0.02))
        self.updateGuiScore()
        self.frame2D.hide()
    ...
    def reportToonHitByBomb(self, avId, moleIndex, timestamp):
        if avId != localAvatar.doId:
            self._DistributedMoleField__showToonHitByBomb(avId, moleIndex, timestamp)
            moleHill = self.moleHills[moleIndex]
    ...
    def _DistributedMoleField__showToonHitByBomb(self, avId, moleIndex, timestamp = 0):
