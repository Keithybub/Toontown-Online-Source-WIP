...
class DistributedInGameEditor(DistributedObject.DistributedObject, Level.Level, InGameEditorElement):
    ...
    def handleImportEntities(self):
        ...
        filename = tkFileDialog.askopenfilename(parent = self.editor.parent, defaultextension = '.egroup', filetypes = [
            ('Entity Group', '.egroup'),
            ('All Files', '*')])
        if len(filename) == 0:
            return None
    ...
    def handleExportEntity(self):
        ...
        import tkFileDialog
        filename = tkFileDialog.asksaveasfilename(parent = self.editor.parent, defaultextension = '.egroup', filetypes = [
            ('Entity Group', '.egroup'),
            ('All Files', '*')])
        if len(filename) == 0:
            return None
