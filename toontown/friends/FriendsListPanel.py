...
class FriendsListPanel(DirectFrame, StateData.StateData):
    ...
    def load(self):
        ...
        self.title = DirectLabel(parent = self, relief = None, text = '', text_scale = TTLocalizer.FLPtitle, text_fg = (0, 0.10000000000000001, 0.40000000000000002, 1), pos = (0.0070000000000000001, 0.0, 0.20000000000000001))
    ...
    def makeFriendButton(self, friendTuple, colorChoice = None, bold = 0):
        ...
        db = DirectButton(relief = None, text = friendName, text_scale = fontScale, text_align = TextNode.ALeft, text_fg = fg, text_shadow = bg, text1_bg = self.textDownColor, text2_bg = self.textRolloverColor, text3_fg = self.textDisabledColor, text_font = fontChoice, textMayChange = 0, command = command, extraArgs = [
            thing,
            showType])
        if playerId:
            accountName = DirectLabel(parent = db, pos = Vec3(-0.02, 0, 0), text = rolloverName, text_fg = (0, 0, 0, 1), text_bg = (1, 1, 1, 1), text_pos = (0, 0), text_scale = 0.044999999999999998, text_align = TextNode.ARight)
    ...
    def enter(self):
        ...
        self.accept(OTPGlobals.PlayerFriendAddEvent, self._FriendsListPanel__friendsListChanged)
        self.accept(OTPGlobals.PlayerFriendUpdateEvent, self._FriendsListPanel__friendsListChanged)
    ...
    def _FriendsListPanel__friendOnline(self, doId, commonChatFlags, whitelistChatFlags):
        if self.panelType == FLPOnline:
            self._FriendsListPanel__updateScrollList()
